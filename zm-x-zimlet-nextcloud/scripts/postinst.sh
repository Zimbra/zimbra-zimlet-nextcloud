...
if [ $? -ne 0 ]; then
   ...
   echo "*** zmzimletctl deploy /opt/zimbra/zimlets-network/zm-x-zimlet-nextcloud.zip ***"
   echo "*** zmprov fc zimlet ***"
else
   echo "*** Deploying Zimbra-Zimlet-Nextcloud zimlet ***"
   su - zimbra -c  "zmzimletctl deploy /opt/zimbra/zimlets-network/zm-x-zimlet-nextcloud.zip"
